...
| * Based on OMPL UncertainKinodynamicPlanner                                                                  |
| * Date : 2019 11 15 Fri                                                              |
...
ompl::control::UncertainKinodynamicPlanner::UncertainKinodynamicPlanner(const SpaceInformationPtr &si) : base::Planner(si, "UncertainKinodynamicPlanner")
{
    ...
    Planner::declareParam<double>("selection_radius", this, &UncertainKinodynamicPlanner::setSelectionRadius, &UncertainKinodynamicPlanner::getSelectionRadius, "0.:.1:"
                                                                                                                "100");
    Planner::declareParam<double>("pruning_radius", this, &UncertainKinodynamicPlanner::setPruningRadius, &UncertainKinodynamicPlanner::getPruningRadius, "0.:.1:100");
}

ompl::control::UncertainKinodynamicPlanner::~UncertainKinodynamicPlanner()
{
    freeMemory();
}

void ompl::control::UncertainKinodynamicPlanner::setup()
{
    base::Planner::setup();
    if (!nn_)
...
void ompl::control::UncertainKinodynamicPlanner::clear()
{
    Planner::clear();
    sampler_.reset();
...
void ompl::control::UncertainKinodynamicPlanner::freeMemory()
{
    if (nn_)
    {
...
ompl::control::UncertainKinodynamicPlanner::Motion *ompl::control::UncertainKinodynamicPlanner::selectNode(ompl::control::UncertainKinodynamicPlanner::Motion *sample)
{
...
ompl::control::UncertainKinodynamicPlanner::Witness *ompl::control::UncertainKinodynamicPlanner::findClosestWitness(ompl::control::UncertainKinodynamicPlanner::Motion *node)
{
...
ompl::base::PlannerStatus ompl::control::UncertainKinodynamicPlanner::solve(const base::PlannerTerminationCondition &ptc)
{
    checkValidity();
    ...
    while (ptc == false){
        ...
        unsigned int propCd = siC_->propagateWhileValid(nmotion->state_, rctrl, cd, rstate);//*************Segmentation error point**************//

        if (propCd == cd){
            base::Cost hCost = opt_->costToGo(rstate, goal);
            base::Cost incCost = opt_->motionCost(nmotion->state_, rstate);
            base::Cost cost = opt_->combineCosts(opt_->combineCosts(nmotion->accCost_, incCost), hCost);
...
void ompl::control::UncertainKinodynamicPlanner::getPlannerData(base::PlannerData &data) const
{
